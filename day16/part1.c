...
int max_pressure_release(int time_left, int location, int current_release, int current_max){
    int new_release;
    ...
    for (int i = 0; i < valve_count; ++i) {
        if(!used[i] && distance[location][i] < time_left){
            used[i] = 1;
            new_release = max_pressure_release(time_left - distance[location][i] - 1, i,
                                                current_release + (time_left - distance[location][i] - 1)*flowrates[i], current_max);
            used[i] = 0;
...
int main(int argc, char ** argv) {
    ...
    printf("%d\n", max_pressure_release(NR_OF_MINUTES, 0, 0, 0));
