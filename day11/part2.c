...
Monkey * read_monkey(FILE *file) {
    ...
    while(fscanf(file, "%ld", &longtemp) == 1){
        add_list(monkey->items, (void*)(size_t)longtemp);
...
void inspect(Monkey* monkey, long* worry_level){
...
int main(int argc, char ** argv) {
    ...
    for (int i = 0; i < monkeys->size; ++i) {
        divisor_product *= ((Monkey*)get_list(monkeys, i))->divisor;
    }

    //calculation
    long inspection_count[10] = { 0 };
    ...
    for (int round = 0; round < NR_OF_ROUNDS; ++round) {
        for (int monkey_nr = 0; monkey_nr < monkeys->size; ++monkey_nr) {
            ...
            for(int item_index = 0; item_index < ((Monkey*)get_list(monkeys, monkey_nr))->items->size; item_index++){
                worry_level = (long)(size_t)get_list(current_monkey->items, item_index);
                inspect(current_monkey, &worry_level);
                inspection_count[monkey_nr]++;
                if(worry_level % (current_monkey)->divisor == 0){
                    add_list(((Monkey*)get_list(monkeys, (current_monkey)->monkeytrue))->items, (void*)(size_t)(worry_level % divisor_product));
                } else {
                    add_list(((Monkey*)get_list(monkeys, (current_monkey)->monkeyfalse))->items, (void*)(size_t)(worry_level % divisor_product));
                }
            }
    ...
    long max_inspection_count = 0;
    long max_inspection_count2 = 0;
    for (int i = 0; i < monkeys->size; ++i) {
    ...
    printf("The level of monkey business is %ld = %ld * %ld\n", max_inspection_count * max_inspection_count2, max_inspection_count, max_inspection_count2);
